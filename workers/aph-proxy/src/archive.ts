...
export async function pollAndArchive(env: Env): Promise<{
  ...
  for (const feed of APH_FEEDS) {
    try {
      ...
      if (!res.ok) {
        perFeed.push({ feed: feed.url, ok: false, new: 0, seen: 0, error: `HTTP ${res.status}` });
        continue;
      }
...
export async function queryArchive(env: Env, params: URLSearchParams): Promise<{
  ...
  if (q) { where.push("LOWER(title) LIKE ?"); binds.push(`%${q.toLowerCase()}%`); }
...
export async function watchlistAnalytics(env: Env, params: URLSearchParams): Promise<{
  ...
  for (const term of terms) {
    const where: string[] = ["LOWER(title) LIKE ?"];
