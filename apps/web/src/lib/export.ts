...
export function buildBriefMarkdown(
  ...
  if (signal.scoringExplanation) {
    lines.push("## Scoring note");
    lines.push(signal.scoringExplanation);
    lines.push("");
  }
  ...
  lines.push("");
  lines.push("---");
  lines.push(`Classification: ${classification} · Source: Parliament of Australia website. Generated by Parliament Pulse.`);
  return lines.join("\n");
}
